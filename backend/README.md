# Backend · Gestão de Ordens de Serviço

API REST em ASP.NET Core 8, Entity Framework Core e SQL Server. Código organizado em Domain (entidades e transições), Application (contratos e serviços de aplicação) e Infra (persistência e migrations).

Todos os comandos deste documento partem da pasta `backend/`. Na raiz do repositório, execute `cd backend`. Consulte também a [visão geral](../README.md) e o [frontend](../frontend/README.md).
...
O workflow [`backend-ci.yml`](../.github/workflows/backend-ci.yml) executa em push, pull request e acionamento manual. Ele restaura dependências, confere formatação, compila em Release, executa testes e disponibiliza os resultados TRX por sete dias. Usa apenas permissão de leitura do repositório e não exige credenciais de produção.
...
O SQL Server local usa o banco `ServiceOrders`, a porta `127.0.0.1:1433` e o volume persistente `ordemdeserviso_os-data`. O Compose mantém o nome de projeto `ordemdeserviso` para reutilizar o volume existente após a organização das pastas.

As migrations estão em `src/Os.Api/Infra/Migrations` e incluem a criação inicial, IDs gerados pelo domínio, sessões de renovação e exclusão lógica. Para aplicar as migrations pendentes:

```powershell
.\scripts\Update-Database.ps1 -StartDatabase
```

Aguarde o SQL Server aceitar conexões antes de aplicar as migrations. O script lê `DB_PASSWORD`, `ConnectionStrings__Database` e `Jwt__Key` de `backend/.env`, quando disponível. Copie `.env.example` para `.env` e substitua os placeholders. A API não carrega esse arquivo automaticamente; use o script ou exporte as variáveis no PowerShell conforme a seção de execução. Não há credenciais padrão versionadas.

Migrations já aplicadas não são executadas novamente. Backups, resultados de testes e configurações locais permanecem dentro de `backend/` e fora do Git.
...
```powershell
.\scripts\Test-Health.ps1 -BaseUrl http://localhost:5080 -MaxLatencyMs 2000
```
...
O GitHub Actions compila a ferramenta, valida sintaxe dos scripts e inclui os testes de logout, métricas e backup/restauração do SQL Server. Os testes de integração e de restauração exigem SQL Server disponível. Consulte a execução do workflow correspondente ao commit para verificar seu resultado.
